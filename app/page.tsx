...
export default function LandingPage() {
  ...
  const statPills = [
    { icon: "mic-outline" as const, label: t("web:landing.statInterviews", { defaultValue: "10,000+ interviews practiced" }) },
    { icon: "briefcase-outline" as const, label: t("web:landing.statJobs", { defaultValue: "5,000+ jobs matched" }) },
    { icon: "edit-2-outline" as const, label: t("web:landing.statResumes", { defaultValue: "8,500+ resumes built" }) },
  ];

  const howItWorks = [
    {
      step: "1",
      title: t("web:landing.step1Title", { defaultValue: "Tell us where you're headed" }),
      description: t("web:landing.step1Description", { defaultValue: "Share your goals, desired roles, and target countries in a two-minute onboarding." }),
      icon: "compass-outline" as const,
    },
    {
      step: "2",
      title: t("web:landing.step2Title", { defaultValue: "Practice with your AI coach" }),
      description: t("web:landing.step2Description", { defaultValue: "Run mock interviews, coding drills, and real scenarios — with feedback after every session." }),
      icon: "message-circle-outline" as const,
    },
    {
      step: "3",
      title: t("web:landing.step3Title", { defaultValue: "Apply with confidence" }),
      description: t("web:landing.step3Description", { defaultValue: "Ship a stronger resume, get daily job alerts, and track your progress on one roadmap." }),
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-14 pb-16">
        ...
        <section>
          <div className="flex items-center gap-3 rounded-pill border border-border bg-surface-2 px-4 py-3 shadow-sm">
            ...
            <input
              type="text"
              disabled
              placeholder={t("web:landing.searchPlaceholder", { defaultValue: "Search mock interviews, courses, tools…" })}
        ...
        <section className="flex flex-col gap-4">
          <div className="flex items-end justify-between">
            <h2 className="text-xl font-bold text-primary">{t("web:landing.popularTools", { defaultValue: "Popular Tools" })}</h2>
          </div>
        ...
        {/* How it works */}
        <section className="flex flex-col gap-4">
          <h2 className="text-xl font-bold text-primary">{t("web:landing.howItWorksTitle", { defaultValue: "How it works" })}</h2>
            ...
            {howItWorks.map((item) => (
              <div key={item.step} className="rounded-card border border-border bg-surface-2 p-5 shadow-sm">
                ...
                <p className="mt-3 text-xs font-semibold uppercase tracking-wide text-hint">
                  {t("web:landing.stepLabel", { defaultValue: "Step {{step}}", step: item.step })}
                </p>
        ...
        <section className="flex flex-col items-center gap-4 rounded-card border border-border bg-gradient-to-br from-brand/10 via-accent-purple/5 to-transparent px-6 py-10 text-center">
          ...
          <LinkButton href="/register" size="lg">
            {t("web:landing.bottomCtaButton", { defaultValue: "Create your free account" })}
          </LinkButton>
        </section>
      </div>
